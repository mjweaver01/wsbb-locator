...
export function CoachProfileAccess({ apiBase }: CoachProfileAccessProps) {
  ...
  async function requestCode(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await response.json()) as { message?: string; error?: string; debugCode?: string }
      if (!response.ok) throw new Error(data.error ?? 'Could not request code')
      setRequestStatus(data.debugCode ? `${data.message} (debug code: ${data.debugCode})` : data.message ?? 'Code sent')
    } catch (err) {
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!meResponse.ok) throw new Error('Saved, but failed to reload profile')
      const meData = (await meResponse.json()) as MeResponse
      hydrateProfile(meData)
  ...
  return (
    <section className="coach-access">
      <div className="coach-access__inner">
        ...
        {!me ? (
          <div className="coach-access__auth-grid">
            ...
            <form className="coach-access__panel" onSubmit={verifyCode}>
              ...
              <button type="submit" disabled={loading || email.trim() === ''}>Verify</button>
            </form>
          </div>
        ) : (
          <form className="coach-access__panel coach-access__panel--full" onSubmit={saveProfile}>
            ...
            <div className="coach-access__actions">
              ...
              <button type="button" className="coach-access__ghost" onClick={logout} disabled={loading}>
                Sign out
              </button>
            </div>
          </form>
        ...
        {requestStatus && <p className="coach-access__status">{requestStatus}</p>}
